...
/* test avl tree */
static AVL_TREE test_avl_tree = NULL;

/* test red black node data */
static struct test_rb_node *rb_node_data;

/* test avl node data */
static struct test_avl_node *avl_node_data;
...
int avl_compare(AVL_NODE *node, void *key)
{
    struct test_avl_node *test_node = (struct test_avl_node *)node;
    KEY *key_val = (KEY *)key;
    KEY node_val = test_node->key;
...
int test_avl_insert(AVL_TREE *root, int index)
{
    struct test_avl_node *test_node = &avl_node_data[index];
    KEY key = test_node->key;

    return avl_insert(root, &test_node->avl_node, &key, avl_compare);
}
...
void test_avl_delete(AVL_TREE *root, int index)
{
    avl_delete(root, &test_keys[index], avl_compare);
}
...
static void parse_params(int argc, char *argv[])
{
    ...
    printf("-----------------------------------------------------------\n");
    printf("          AVL Tree and Red Black Tree test demo            \n");
...
void test_data_build(int show)
{
    ...
    avl_node_data = (struct test_avl_node *)
        malloc(sizeof(struct test_avl_node) * nodes_num);
    if (avl_node_data == NULL) {
        printf("Build test avl tree failed, no enough memory, nodes_num is %d\n",
            nodes_num);
        exit(1);
    }
    ...
    for(i = 0; i < nodes_num; ++i) {
        ...
        avl_node_data[i].key = test_keys[i];
    }
...
void func_test()
{
    ...
    for (i = 0; i < nodes_num; ++i)
    {
        test_avl_insert(&test_avl_tree, i);
#if 0
        printf("== Add node: %d\n", test_keys[i]);
        printf("== Tree detail: \n");
        test_rb_print(&test_rb_tree);
...
//#if CHECK_DELETE
    for (i = 0; i < nodes_num; ++i) {
        test_avl_delete(&test_avl_tree, i);
#if 0
        printf("== Del node: %d\n", test_keys[i]);
        printf("== Tree detail: \n");
        test_rb_print(&test_rb_tree);
...
void perf_test()
{
	...
	for (i = 0; i < perf_loops; i++) {
        ...
        time1 = ros_rdtsc();
		for (j = 0; j < nodes_num; j++) {
			test_avl_insert(&test_avl_tree, j);
		}
        time2 = ros_rdtsc();
        cost1 = time2 - time1;
        ...
        time1 = ros_rdtsc();
		for (j = 0; j < nodes_num; j++) {
			test_avl_delete(&test_avl_tree, j);
		}
        time2 = ros_rdtsc();
        cost2 = time2 - time1;

        printf("[avl]i:%d, insert cost:%lu, delete cost:%lu.\n",
            i, cost1, cost2);
        test_data_free();
        printf("-----------------------------------------------------------\n");
	}
